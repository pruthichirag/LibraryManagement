...
$a2=array();
$arw2=mysql_query("SELECT BOOK_ID FROM books WHERE BOOK_ID IN(SELECT bkid FROM issu1 WHERE usn='$username')"); 
...
$rd = mysql_fetch_array($dtq1);
 $dt1= $rd[0];
$dtq2=mysql_query("SELECT RETURN_DATE FROM issu1 WHERE bkid='$post2[0]'");
...
$fins=mysql_query("UPDATE fine SET amt='$famt' WHERE usn='$username");
...
$fine=0;
$fn9=mysql_query("SELECT amt FROM fine WHERE usn='$username'");
$fnt = mysql_fetch_array($fn9);
 $fine= $fnt[0];

 $acc=$username.'.php';
 $ac_str=$p1.''.$name2.''.$p2.''.$username.''.$p3.''.$image.''.$p4.''.$book1.''.$p5.''.$p6.''.$fine.''.$sp7;
 fwrite($accf,$ac_str);
 header("Location:".$acc);
